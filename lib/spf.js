...
class SPF {
  ...
  async check_host (ip, domain, mail_from, cb) {
    const self = this;
    ...
    try {
      const txt_rrs = await dns.resolveTxt(domain)
      let spf_record;
      let match;
  ...
  mech_all (qualifier, args, cb) {
    cb(null, this.return_const(qualifier));
  }
  ...
  async mech_exists (qualifier, args, cb) {
    const self = this;
    self.count++;
    ...
    try {
      ...
      cb(null, self.return_const(qualifier));
    }
    catch (err) {
  ...
  async mech_a (qualifier, args, cb) {
    const self = this;
    self.count++;
    // Parse any arguments
    let cm;
    ...
    // Use current domain
    try {
      const addrs = await dns[resolve_method](domain)
  ...
  async mech_mx (qualifier, args, cb) {
    const self = this;
    this.count++;
    // Parse any arguments
    let cm;
    ...
    try {
      const mxes = await net_utils.get_mx(domain)
      ...
      for (let a=0; a<mxes.length; a++) {
        ...
        try {
          const addrs = await dns[resolve_method](mx)
          pending--;
      ...
      if (pending === 0) cb(null, self.SPF_NONE);
    }
    catch (err) {
      switch (err.code) {
        case dns.NOTFOUND:
  ...
  async mech_ptr (qualifier, args, cb) {
    const self = this;
    this.count++;
    let dm;
    let domain = this.domain;
    ...
    try {
      const ptrs = await dns.reverse(this.ip)
      let resolve_method;
      ...
      if (pending === 0) cb(null, self.SPF_NONE); // No queries run
    }
    catch (err) {
  ...
  mod_redirect (domain, cb) {
    ...
    this.check_host(this.ip, domain, this.mail_from, cb);
  }
